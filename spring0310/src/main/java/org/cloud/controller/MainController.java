...
public class MainController {
	...
	public String login(MemberDTO member, HttpSession session, RedirectAttributes rttr) throws Exception {
		...
		if (user != null) { // 만약 있으면 (로그인 성공 시)
			...
			return "redirect:/"; // 로그인이 성공했으니 메인 페이지로 이동
		}
		rttr.addFlashAttribute("errorMsg", "아이디 또는 비밀번호가 일치하지 않습니다."); // 로그인 실패 시 실행
	...
	@PostMapping("/join")
	public String memberJoin(MemberDTO member) throws Exception {
